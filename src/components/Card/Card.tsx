import React from 'react';
import './Card.css';
import {IonButton, IonCard, IonCardContent, IonCardHeader, IonCardSubtitle, IonCardTitle, IonImg} from '@ionic/react';


type Props = {
    isHome?: boolean
}

function Card({isHome}: Props) {

    const data = [
        {
            name: "Orlen",
            imgUrl: "https://lh3.googleusercontent.com/p/AF1QipNxl1JnhB4iREk-PImPgG0aymmvWLCm_J0ATh-6=s1360-w1360-h1020",
            type: "commercial",
            address: "ul. Struga 10",
            location: "Szczecin",
            rating: 3
        },
        {
            name: "McDonalds",
            imgUrl: "https://lh3.googleusercontent.com/p/AF1QipNeh1hGaXOQWEtKfNpALdlhjtaIzo7m_qMH5lFy=s1360-w1360-h1020",
            type: "commercial",
            address: "ul. Struga 24",
            location: "Szczecin",
            rating: 4
        },
        {
            name: "Shell",
            imgUrl: "https://lh5.googleusercontent.com/p/AF1QipNCGLI5doOrYRdIn4eKEbGbZ0rwvrMU0liA4VE=w260-h175-n-k-no",
            type: "commercial",
            address: "ul. Struga 1",
            location: "Szczecin",
            rating: 5
        },
        {
            name: "Wolantariusz",
            imgUrl: "data:image/png;base64,",
            type: "private",
            address: "ul. Struga 27",
            location: "Szczecin",
            rating: 4
        },
    ]

    return (
        <>
            {isHome && data.map((item, index) =>
                <IonCard className="card ion-margin-top" key={index}>
                    <IonImg alt="user" src={item.imgUrl} className="img"/>
                    <IonCardHeader>
                        <IonCardTitle><span>Nazwa:</span>&nbsp;{item.name}</IonCardTitle>
                        <IonCardSubtitle><span>Miasto:</span>&nbsp;{item.location}</IonCardSubtitle>
                    </IonCardHeader>
                    <IonCardContent className="flex ion-justify-content-between">
                        <p><b>📍&nbsp;</b>{item.address}</p>
                        <p><b>Rating:</b>&nbsp;{item.rating}</p>
                    </IonCardContent>
                    <IonButton fill="default">Info</IonButton>
                    <IonButton fill="clear">Zadzwon</IonButton>
                </IonCard>
            )}
        </>
    );
}

export default Card;